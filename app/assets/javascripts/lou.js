...
$('.modal-dialog #new_user').submit(function() {  
  ...
  $.ajax({
    ...
    var newreg = $( "<li><a href='/panel/users/" + data.id + "'>" + data.name + "</a></li>" );
    $( "#clientesrecentes" ).prepend( newreg );
    swal("Cliente adicionado com sucesso!", "Clique para continuar", "success");
    $('#modalcliente').modal('hide');
...
$('.modal-dialog #new_product').submit(function() {  
  ...
  $.ajax({
    ...
    swal("Producto agregado con exito!", "Click para continuar", "success");
    $('#modalproduto').modal('hide');
  }).fail(function(request) {
    console.log("Codigo del error: " + request.status);
    sweetAlert("Ops! Ocurrio un error.", "Por favor, revise las informaciones y vuelva a intentar! Codigo del error: " + request.status, "error");
  });
  return false;
});
